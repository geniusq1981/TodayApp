var calPage = (function() {
    ...
    calPagePlugin.prototype = {
        ...
        active: function() {
            console.log("calpage active!");
            $('#nextMonth').removeClass('next-month-deactive');
            $('#nextMonth').addClass('next-month-active');
        },
        deactive: function() {
            console.log("calpage deactive!");            
            this.removefocus();
        ...
        tpKeyHandler: function(e) {
            ...
            switch (e.keyCode) {
                case 37: //LEFT arrow
                    if (this.farea) {
                        this.moveCalendarFocus("left");
                    } else {
                        this.focusleft();
                        this.curselected.rc = -1;
                        this.curselected.d = -1;
                        $('#prevMonth').click();
                    }
                    break;
                case 38: //UP arrow
                    if (this.farea) {
                        this.moveCalendarFocus("up");
                ...
                case 40: //DOWN arrow
                    if (this.farea) {
                        this.moveCalendarFocus("down");
                    } else {
                        this.selectonedate(this.curselected.d);
                        this.removefocus();
                        this.farea = 1;
                    }
                    break;
        ...
        focusright: function() {
            $('#prevMonth').attr('class', 'prev-month-deactive');
            $('#nextMonth').attr('class', 'next-month-active');
        },
        focusleft: function() {
            $('#prevMonth').attr('class', 'prev-month-active');
            $('#nextMonth').attr('class', 'next-month-deactive');
        },
        removefocus: function() {
            $('#prevMonth').attr('class', 'prev-month-deactive');
            $('#nextMonth').attr('class', 'next-month-deactive');
        },
        moveCalendarFocus: function(direction) {
            this.selected = $('.selected');
            this.curselected.rc = parseInt(this.selected.attr("rc"));
            this.curselected.d = this.selected.attr("data");
            console.log(this.curselected);
            switch (direction) {
                case 'left':
                    if (this.curselected.rc % 10 <= 0) {
                        //$('#prevMonth').click();
                        var that = this;
                        calendarplugin.toPrevMonth(function() {
                            that.curselected.rc = -1;
                            that.curselected.d = -1;
                    ...
                    break;
                case 'right':
                    if (this.curselected.rc % 10 >= 6) {
                        var that = this;
                        calendarplugin.toNextMonth(function() {
                            that.curselected.rc = -1;
                            that.curselected.d = -1;
                            console.log("!!!Testlsf");
                            that.refreshselected();
                    ...
                    break;
                case 'up':
                    if (this.curselected.rc / 10 < 1) {
                        this.disableselecteddate();
                        this.focusright();
                        this.farea = 0;
                        return;
                    } else {
                        this.curselected.rc -= 10;
                    }
                    break;
                case 'down':
                	console.log(calendarplugin.getRows()-1);
                    if (this.curselected.rc / 10 < (calendarplugin.getRows()-1)) {
                        this.curselected.rc += 10;
                    }
                    break;
                default:
        ...
        selectonedate: function(data) {
            if (data > 0) {
                $('td[data="' + data + '"]').addClass('selected');
                console.log($('td[data="' + data + '"]'));
            } else {

                var a = $(".currentDay"); //document.getElementsByClassName("currentDay");
                console.log(a);
                if (a.length) {
                    var data = a.attr("data");
                    this.curselected.rc = a.attr("rc");
                    this.curselected.d = data;
                    a.addClass('selected');
                    console.log("find" + data);

                } else {
                    var obj = $(".currentMonth");
                    this.curselected.rc = obj.attr("rc");
                    this.curselected.d = obj.attr("data");
                    addClass(obj[0], 'selected');
                    console.log("not find" + data);
                }
            }

        },
        renderCalendarData: function() {
            calendarplugin.update();
        },
        renderdatedetail: function() {
            console.log("renderdetail");
            var D;
            var Dformat;
            console.log(this.curselected.d);
            if (this.curselected.d < 0) {
                D = new Date();

                var _year = D.getFullYear();
                var _month = D.getMonth() + 1; // 月从0开始计数

                Dformat = Calcore.getDateStr(D);
            } else {
            	var selectedEl = $('td[data="' + this.curselected.d + '"]');
            	var classname = selectedEl.attr("class");
            	console.log(selectedEl);
            	console.log(classname);
                if(classname.match("preMonth")== "preMonth") {
                    var that = this;
                    var cursor = that.curselected.d;
                    calendarplugin.toPrevMonth(function() {
                        console.log(cursor);
                        that.selectonedate(cursor);
                    });
                }
                if(classname.match("nextMonth") == "nextMonth") {
                    var that = this;
                    var cursor = that.curselected.d;
                    calendarplugin.toNextMonth(function() {
                        console.log(cursor);
                ...
                Dformat = this.curselected.d;
                var y = this.curselected.d.substr(0, 4);
                var m = this.curselected.d.substr(4, 2);
                m = m < 10 ? (m.substr(1, 1) - 1) : (m - 1);
                var day = this.curselected.d.substr(6, 2);
                D = new Date(y, m, day);
                console.log(D);
            }
            ...
            document.getElementById("calweek").getElementsByTagName("use")[0].href.baseVal = zdicon[D.getDay()];
            document.getElementById("caldate").innerHTML = dd;
            document.getElementById("calndate").innerHTML = Calcore.getCMonth()+Calcore.getCDate();

            var YJdata = Calcore.getYJ();
            document.getElementById("YHuangli").innerHTML = limitYJ(YJdata["suit"]);
            document.getElementById("NHuangli").innerHTML = limitYJ(YJdata["avoid"]);
            document.getElementById("jieri").innerHTML = Calcore.getFestival(); //V[Dformat.substr(4,4)]?V[Dformat.substr(4,4)]:"";
        },
        refreshselected: function() {
            console.log(this.curselected.rc);

            if (this.curselected.rc < 0) {
                $('.selected').removeClass('selected');
                this.selectonedate();
            } else {
                var selectedEl = $('td[rc="' + this.curselected.rc + '"]');
                var classname = selectedEl.attr("class");
                this.curselected.d = $('td[rc="' + this.curselected.rc + '"]').attr("data");
                console.log(this.curselected.d);
                /*if (classname == "currentMonth" || classname == "currentDay") {
                    $('.selected').removeClass('selected');
                    console.log(this.curselected.rc);
                    console.log(this.curselected.d);
                    $('td[rc="' + this.curselected.rc + '"]').addClass('selected');
        ...
        disableselecteddate: function() {
            var a = $(".selected");
            console.log(a);
            a.removeClass('selected');
        }
    }
    return new calPagePlugin();
})();
...
var Calcore = (function() {
    ...
    var YJ = ["嫁娶", "冠笄", "修造", "动土", "作灶", "祈福", "开光", "掘井", "开市", "安葬",
        "祭祀", "交易", "纳财", "斋醮", "开渠", "上梁", "动土", "破土", "订盟", "纳采",
        "冠笄", "会亲友", "行丧", "伐木", "作梁", "开光", "求嗣", "出行", "安床", "出货财",
        "沐浴", "理发", "治病", "合寿木", "安机械", "移徙", "入宅", "捕捉", "结网", "畋猎",
        "坏垣", "求医", "塑绘", "掘井", "断蚁", "余事勿取", "造庙", "纳采", "裁衣", "合帐",
        "立券", "作厕", "栽种"
    ];
    var V = {
        "0101": "元旦",
        ...
        return j.replace(/dd?d?d?|MM?M?M?|yy?y?y?/g, function(k) {
            ...
            switch (k) {
                ...
                case "dd":
                    return G(h.getDate());
                case "d":
                    console.log(h.getDate().toString());
    ...
    function GetLunarDayPlus(formatdate) {
        //solarYear = solarYear<1900?(1900+solarYear):solarYear;
        console.log(formatdate);
        var solarYear = formatdate.substr(0, 4);
        var solarMonth = formatdate.substr(4, 2);
        var solarDay = formatdate.substr(6, 2);
        if (solarYear < 1921 || solarYear > 2050) {
            return "";
        } else {
            solarMonth = (parseInt(solarMonth) > 0) ? (solarMonth - 1) : 11;
    ...
    var zd = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"];
    var zdicon = ["#icon-Mon", "#icon-TUE", "#icon-WED", "#icon-THU", "#icon-FRI", "#icon-SAT", "#icon-SUN"];
    var zm = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"]
    ...
    calcore.prototype = {
        GetLunarDay: function(solarYear, solarMonth, solarDay) {
            ...
            if (solarYear < 1921 || solarYear > 2050) {
                ...
                console.log(cMonth);
                console.log(cDay);
                console.log(cYear * 10000 + cMonth * 100 + cDay);
                sxtwl.getJieQi(solarYear, solarMonth, solarDay);
                return GetcDateString();
            }
        },
        /**
        ...
        getFestival: function() {
            var jieri;
            var YJ, NJ, JQ;
            var dateFormat = this.getDateStr(TheDate).substr(4, 4);
            var cdateFormat = (cYear * 10000 + cMonth * 100 + cDay).toString().substr(4, 4);
            console.log(V[dateFormat] ? V[dateFormat] : "");
            console.log(T[cdateFormat] ? T[cdateFormat] : "");
            var showInLunar;
            showInLunar = sxtwl.getJieQi(TheDate.getFullYear(),TheDate.getMonth()+1,TheDate.getDate());
            /*console.log(c(TheDate.getFullYear(), TheDate.getMonth() * 2));
            console.log(f(TheDate, "d"));
            if (c(TheDate.getFullYear(), TheDate.getMonth() * 2) == f(TheDate, "d")) {
                showInLunar = L[TheDate.getMonth() * 2]
            }
            if (c(TheDate.getFullYear(), TheDate.getMonth() * 2 + 1) == f(TheDate, "d")) {
                console.log("fit");
                showInLunar = L[TheDate.getMonth() * 2 + 1]
            }*/
            console.log(showInLunar);
            YJ = V[dateFormat] ? V[dateFormat] : "";
            NJ = T[cdateFormat] ? T[cdateFormat] : "";
            JQ = showInLunar ? showInLunar : "";
            return YJ ? YJ : (NJ ? NJ : (JQ ? JQ : ""));
        },
        //获取 节气 
        getJieQi: function() {
            var showInLunar;
        ...
        GetLunarDayPlus: function(formatdate) {
            //solarYear = solarYear<1900?(1900+solarYear):solarYear;
            console.log(formatdate);
            var solarYear = formatdate.substr(0, 4);
            var solarMonth = formatdate.substr(4, 2);
        ...
        getYJ: function() {
        	if(YJ_DATA){
        		...
        		//console.log(detail);
        		//console.log(YJ_DATA["2018"]);
        		//console.log(YJ_DATA[year]["date"]);
        		console.log(YJ_DATA[year][detail]);
        		return YJ_DATA[year][detail];
        	}else{        		
        		 var dateformat = this.getDateStr(TheDate);
                 var origin = yjdata[dateformat];
                 if (origin) {
                     return origin;
                 }
                 var count = YJ.length;
                 ...
                 var tmp = new Array;
                 for (var i = 0; i < 6; i++) {
                     tmp[i] = YJ[original[i] - 1];
                 }
                 yjdata[dateformat] = tmp;
                 console.log(tmp);
                 
                 return {"suit":tmp[0]+" "+tmp[1]+" "+tmp[2],"avoid":tmp[3]+" "+tmp[4]+" "+tmp[5]};	
        	}
...
var zd = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"];
var zdicon = ["#icon-SUN", "#icon-MON", "#icon-TUE", "#icon-WED", "#icon-THU", "#icon-FRI", "#icon-SAT"];
var zm = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"]
...
var sxtwl = (function() {
	...
	*****/
    ...
    var sxtwlplugin = function() {
        this.calendarData = new Array(0xA4B, 0x5164B, 0x6A5, 0x6D4, 0x415B5, 0x2B6, 0x957, 0x2092F, 0x497, 0x60C96, 0xD4A, 0xEA5, 0x50DA9, 0x5AD, 0x2B6, 0x3126E, 0x92E, 0x7192D, 0xC95, 0xD4A, 0x61B4A, 0xB55, 0x56A, 0x4155B, 0x25D, 0x92D, 0x2192B, 0xA95, 0x71695, 0x6CA, 0xB55, 0x50AB5, 0x4DA, 0xA5B, 0x30A57, 0x52B, 0x8152A, 0xE95, 0x6AA, 0x615AA, 0xAB5, 0x4B6, 0x414AE, 0xA57, 0x526, 0x31D26, 0xD95, 0x70B55, 0x56A, 0x96D, 0x5095D, 0x4AD, 0xA4D, 0x41A4D, 0xD25, 0x81AA5, 0xB54, 0xB6A, 0x612DA, 0x95B, 0x49B, 0x41497, 0xA4B, 0xA164B, 0x6A5, 0x6D4, 0x615B4, 0xAB6, 0x957, 0x5092F, 0x497, 0x64B, 0x30D4A, 0xEA5, 0x80D65, 0x5AC, 0xAB6, 0x5126D, 0x92E, 0xC96, 0x41A95, 0xD4A, 0xDA5, 0x20B55, 0x56A, 0x7155B, 0x25D, 0x92D, 0x5192B, 0xA95, 0xB4A, 0x416AA, 0xAD5, 0x90AB5, 0x4BA, 0xA5B, 0x60A57, 0x52B, 0xA93, 0x40E95);
        this.currentyear;
        this.JieQi = {};
        this.zqTable=new Array();
        this.jqTable=new Array();
        this.hsTable=new Array();
    }
    sxtwlplugin.prototype = {
        setYear: function(solarYear) {
        	if(solarYear==this.currentyear)return;
        	y=solarYear;
      	    var zq=new Array(),jq=new Array(), hs=new Array();  //中气表,节气表,日月合朔表
      	  ...
      	  for(i=0;i<tot;i++){
      	    var zm=(i*2+18)%24, jm=(i*2+17)%24; //中气名节气名
      	    JDate.setFromJD(jq[i]+J2000+8/24,1); this.jqTable[i]=JDate.toStr();/* out+=jqB[jm]+":"+this.jqTable[i]+" ";*/ this.JieQi[this.jqTable[i].split(" ")[1]]=jqB[jm]+"("+ this.jqTable[i].split(" ")[2]+")"; //显示节气
      	    JDate.setFromJD(zq[i]+J2000+8/24,1); this.zqTable[i]=JDate.toStr();/*out+=jqB[zm]+":"+this.zqTable[i]+" "; */ this.JieQi[this.zqTable[i].split(" ")[1]]=jqB[zm]+"("+this.zqTable[i].split(" ")[2]+")" ;//显示中气
      	    JDate.setFromJD(hs[i]+J2000+8/24,1); this.hsTable[i]=JDate.toStr();/*out+=yn[i]  +":"+ this.hsTable[i]+"\r\n";*/ //显示日月合朔
      	    console.log(this.jqTable[i]);
      	  }
      	 
      	  this.currentyear=solarYear;
        },
        //获取 节气 
        getJieQi: function(solarYear,solarMonth,solarDay) {
            var showInLunar;
            this.setYear(solarYear);
            console.log(this.JieQi);
            solarMonth="0"+solarMonth;
            solarDay = "0"+solarDay;
            var day=solarYear+"-"+solarMonth.substr(solarMonth.length-2,2)+"-"+solarDay.substr(solarDay.length-2,2);
            /*for(var i=0;i<12;i++){
            	var day1 = this.jqTable[i].split(" ");
            	var day2 = this.zqTable[i].split(" ");
            	console.log(day1);
            	console.log(day2);
            	console.log(day);
            	if(day==day1[1]){
            		showInLunar = jqB[(i*2+17)%24]+"("+day1[2]+")";            		
            	}
            	if(day==day2[1]){
            		showInLunar = jqB[(i*2+18)%24]+"("+day2[2]+")";
            	}
            }*/
            showInLunar = this.JieQi[day];
            console.log(showInLunar);
            if(showInLunar=="undefined"){showInLunar="";}
            return showInLunar;
        }
    }
    return new sxtwlplugin();
})();